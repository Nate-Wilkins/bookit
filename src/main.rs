...
fn command_view(args: clap::ArgMatches) -> Result<()> {
  // Load config.
  let config_path =
    &std::path::PathBuf::from(shellexpand::tilde(args.value_of("config").unwrap()).into_owned());
...
fn command_edit(args: clap::ArgMatches) -> Result<()> {
  ...
  let bookit_edit_command_expanded = String::from(shellexpand::env(&bookit_edit_command).unwrap());
  let bookit_edit_command_parts: Vec<String> = shlex::split(&bookit_edit_command_expanded).unwrap();
